...
## 3. 对话套路（可直接复制改词）
- 读上下文：`请先看 go.mod、cmd/api/main.go、frontend/package.json，总结依赖与入口。`
...
### 4.3 后端开发（让 Codex 按步骤完成）
1) 读 go.mod、main.go，确认依赖与入口；输出 4~6 步计划。
2) 建骨架：`main.go` 包含 DB 初始化（sqlite）、模型 User/Role/Permission、路由、基础中间件。
3) 认证：`/signup` + bcrypt，`/login` + JWT；`authMiddleware` 解析 Bearer；`requirePerm` 校验权限。
4) RBAC 接口：`/roles`、`/permissions` 创建；`/roles/:id/permissions` 分配；`/users/:id/roles` 分配。
5) 受保护示例：`/demo` 需 `perm:demo:read`；`/me` 返回用户/角色/权限。
6) 种子数据与测试：初始化 admin 用户、默认权限；运行 `go test ./...`（或写 1-2 个 httptest 覆盖 `/login`、`/demo`）。

> 需要参考代码时，可让 Codex 输出完整 `main.go`（与前一版类似），并保持同路径。

### 4.4 前端开发（Web/H5，同一代码基）
1) 读 `frontend` 结构，计划 4~6 步：路由、状态、API 层、页面、样式。
2) 配置基础：启用 Tailwind（或保持默认样式），设置全局布局、移动端断点（如 `max-width: 768px`）。
3) 状态与 API：
   - `src/api/http.ts`：Axios 实例、拦截器写入 Bearer token。
   - `src/stores/auth.ts`：登录/注销、token/用户信息存储；持久化可用 localStorage。
   - `src/stores/rbac.ts`：角色/权限列表、分配动作。
4) 路由与守卫：
   - 路由：`/login`、`/demo`、`/roles`、`/permissions`、`/users`。
   - 守卫：若无 token，跳转 `/login`；可在 meta 中声明 `permission`，由全局守卫校验（调用 `/me` 或缓存）。
5) 页面：
   - 登录页：表单提交 `/login`，成功存 token，跳转 `/demo`。
   - Demo 页：调用 `/demo`，展示受保护数据；失败提示无权限。
   - 角色/权限管理：表单创建角色/权限；列表 + 分配按钮（可用简单表格）。
   - 用户角色分配：输入用户 ID -> 角色；或先列出用户（可选）。
6) 适配与构建：
   - 响应式：使用 flex/grid + Tailwind 断点；按钮/表单在窄屏堆叠。
   - 构建：`npm run build && npm run preview`；如在 H5 容器，保持相对路径与移动端视口 `<meta viewport>`（Vite 默认）。
...
- 安装与初始化：`npm install -g @fission-ai/openspec@latest` → `openspec --version` → 在项目根执行 `openspec init`。会生成 `openspec/` 目录与项目根的 `AGENTS.md`，并为支持的助手写入 `/openspec-*` 命令（Codex 原生支持）。
